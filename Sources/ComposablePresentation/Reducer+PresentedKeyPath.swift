...
extension Reducer {
  /// Combines the reducer with another reducer that works on `@Presented<LocalState>`.
  ///
  /// - All effects returned by another reducer will be canceled when `LocalState` becomes `nil`.
  ///
  /// - Parameters:
  ///   - localReducer: A reducer that works on `Presented<LocalState>`, `LocalAction`, `LocalEnvironment`.
  ...
  ///   - toLocalState: A key path that can get/set `Presented<LocalState>` inside `State`.
  ...
  public func presented<LocalState, LocalAction, LocalEnvironment>(
